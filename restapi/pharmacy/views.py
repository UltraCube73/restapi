...
class MedicationView(APIView):
    serializer_class = MedicationSerialiser
    
    def get(self, request):
        if request.GET.get('id') != None:
            medications = Medication.objects.filter(id=request.GET.get('id')).first()
            serializer = self.serializer_class(medications)
    ...
    def post(self, request):
        ...
        serializer = self.serializer_class(data=medication)
    ...
    def put(self, request):
        ...
        serializer = MedicationSerialiser(instance=medication_saved, data=data, partial=True)
...
class PharmacyView(APIView):
    serializer_class = PharmacySerialiser
